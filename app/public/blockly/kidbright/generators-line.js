...
Blockly.JavaScript['linenotify_set_access_token'] = function(block) {
	...
	var code = 'kb_line.line_token(' + FilterCode(value_token) + ');\n';
	return code;
};
...
Blockly.JavaScript['linenotify_send_notifications'] = function(block) {
	...
	code += 'kb_line.line_msg(' + FilterCode(value_message) + '); ';
	code += 'kb_line.line_img_1(' + FilterCode(value_imagethumbnail) + '); ';
	code += 'kb_line.line_img_2(' + FilterCode(value_imagefullsize) + '); ';
	code += 'kb_line.line_stk_pack(' + FilterCode(value_stickerpackageid) + '); ';
	code += 'kb_line.line_stk_id(' + FilterCode(value_stickerid) + '); ';
	//code += 'DEV_IO.LINENotify().notify();\n';
	return code;
};
